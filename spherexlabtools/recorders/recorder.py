...
class HDF5Recorder(QueueThread):
    ...
    def handle_old(self, record):
        ...
        if "group_records" in kwargs:
            self.record_size = kwargs.pop("group_records")
            if not self.record_ind < self.record_size:
                write_params = True
                self.records += 1
                self.record_ind = 1
            else:
            ...
            write_params = True
            self.records += 1
        self.store.put(self.META_GROUP, pd.Series({"Records": self.records}), data_columns=True)
        ...
        data = record.data
        shape = data.shape
        index = pd.MultiIndex.from_product([[self.records], [self.record_group_ind], np.arange(shape[0])],
                                           names=["Record", "RecordInd", "Row"])
        df = pd.DataFrame(data, index=index)
        #df.sort_index(inplace=True)
        self.store.append(group, df, **kwargs)
