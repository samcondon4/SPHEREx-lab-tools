...
class SRS830m(Instrument):
    ...
    def __init__(self, gpib_id="GPIB0::8::INSTR"):
        ...
        self.add_get_parameter("time constant", self.get_time_constant)
        self.add_set_parameter("time constant", self.set_time_constant)
    ...
    async def measure(self, sample_time):
        cur_params = await self.get_parameters("All")
        fs = cur_params["sample rate"]
        tc = cur_params["time constant"]
        data_array = np.zeros(int(sample_time*fs) + 1)
        ...
        for i in range(len(data_array)):
            data_read = self.inst.read_bytes(4)
            print(data_read)
            #data_array[i] = self.inst.read_bytes(4).decode("utf-8")
