...
class SR830(Instrument, Housekeeping):
    ...
    def __init__(self, gpib_id="GPIB0::8::INSTR", log_path=".\\housekeeping\\Lockin.csv"):
        ...
        # Status registers ###############
        self.add_get_parameter("lia status", self.get_lia_status)
        self.add_get_parameter("error status", self.get_error_status)
        #########################################################################################
        ...
        self.set_log_method("Lockin", self.measure_hk)
        ############################################################
