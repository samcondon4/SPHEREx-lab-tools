...
hardcoded_list = {'Marco': {'sms': False, 'email': False},
...
limit_dict = {'80k': {'<': 80}, '20k': {'<': 80}, 'ts': {'<': 10}, 'delta': {'>': 150}}
...
class alertMessagingWindow(Ui_MainWindow):
    ...
    def __init__(self, wait=0.5):
        ...
        #self.pushButton_Test.clicked.connect(self.sendTestMessage)
        ...
        #self.listView_table.doubleClicked.connect(self.loopThrough)
    ...
    def addToSettingsDict(self, signalIn):
        ...
        if key not in self.cdict:
            ...
            dictIn['limit_val'] = list([val0.values() for key0, val0 in limit_dict.items() if key0 in key][0])[0]
...
class conditionHorizontalLine(QtWidgets.QWidget, Thread):
    ...
    def __init__(self, dictIn):
        ...
        self.textEdit_current.setMinimumSize(QtCore.QSize(130, 35))
        self.textEdit_current.setMaximumSize(QtCore.QSize(130, 35))
        ...
        self.comboBox_Limit.setCurrentText(self.settingsDict['condition'])
        ...
        self.doubleSpinBox.setMinimumSize(QtCore.QSize(110, 35))
        self.doubleSpinBox.setMaximumSize(QtCore.QSize(110, 35))
        self.doubleSpinBox.setObjectName("doubleSpinBox")
        self.doubleSpinBox.setMaximum(1000.0)
        self.doubleSpinBox.setValue(self.settingsDict['limit_val'])
        self.horizontalLayout.addWidget(self.doubleSpinBox)
        self.comboBox_action1 = QtWidgets.QComboBox(self)
        font = QtGui.QFont()
        font.setPointSize(12)
        self.comboBox_action1.setFont(font)
        self.comboBox_action1.setObjectName("comboBox_action1")
        self.comboBox_action1.addItem("")
        self.comboBox_action1.addItem("")
        self.comboBox_action1.addItem("")
        self.comboBox_action1.setItemText(0, "Email + SMS")
        self.comboBox_action1.setItemText(1, "Email")
        self.comboBox_action1.setItemText(2, "SMS")
        self.horizontalLayout.addWidget(self.comboBox_action1)
        self.textEdit_status = QtWidgets.QTextEdit(self)
        self.textEdit_status.setFont(font)
        self.textEdit_status.setMinimumSize(QtCore.QSize(130, 35))
        self.textEdit_status.setMaximumSize(QtCore.QSize(130, 35))
        ...
        self.comboBox_action1.currentIndexChanged.connect(self.update_settings_dict)
    ...
    def update_settings_dict(self):
        self.settingsDict['condition'] = self.comboBox_Limit.currentText()
        self.settingsDict['limit_val'] = self.doubleSpinBox.value()
        try:
    ...
    def update_last_value(self):
        ...
        with rec_lock:
            if self.settingsDict['limit_val']:
                ...
                if deltaTime < crashTrigger:
                    ...
                    if self.checkBox_field.checkState() and not self.settingsDict['alert_sent']:
                        #print('checking now {}'.format(datetime.now()))
                        if self.settingsDict['condition'] == '<':
                            if m > self.doubleSpinBox.value():
                                ...
                                #loggingBox_text = "{} - Field {} ALERT SENT.".format(
                                #    datetime.now().strftime("%Y/%m/%d at %M:%H:%S"), self.settingsDict['field_name'])
                                #self.textEdit_loggingBox.append(loggingBox_text)
                        else:
                            if m < self.doubleSpinBox.value():
                                alert_text = self.get_alert_message()
                    ...
                    alert_text = "{} appears to have crashed on {} at {}".format(self.settingsDict['server']['ssh_host'], datetime_now.strftime("%Y/%m/%d"), datetime_now.strftime("%H:%M:%S"))
                    print(alert_text)
                    alertMessagingWindow.sendAlertMessage(text=alert_text)
                else:
    ...
    def get_alert_message(self):
        ...
        condition = self.settingsDict['condition']
        limit = self.settingsDict['limit_val']
        mess = "You are getting this alert because at {3}: {0} is {1} {2}".format(field, condition, limit, datetime.now().strftime("%Y%m%d_%H%M%S"))
        return mess
